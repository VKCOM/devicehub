...
def test_missing_authorization_header(base_url):
    """Test authentication failure with no authorization header"""
    client = AuthenticatedClient(base_url=base_url, token="")
    response = get_users.sync_detailed(client=client)
    equal(response.status_code, 401)
...
def test_privilege_enforcement_admin_only(api_client_custom_token, service_user_creating):
    ...
    regular_user = service_user_creating(privilege='user')
