...
class TestTeamErrorHandling:
    """Test suite for team operation error scenarios"""

    def test_delete_nonexistent_team(self, api_client, unsuccess_response_check, random_str):
        """Test deletion of non-existent team"""
        fake_team_id = f'nonexistent-{random_str()}'
        response = delete_team.sync_detailed(client=api_client, id=fake_team_id)
        unsuccess_response_check(response, status_code=404, description='Not Found (team)')

    def test_update_nonexistent_team(self, api_client, unsuccess_response_check, random_str):
        """Test updating non-existent team"""
        fake_team_id = f'nonexistent-{random_str()}'
        response = update_team.sync_detailed(
            client=api_client,
            id=fake_team_id,
            body=TeamPayload(name=f'updated-{random_str()}')
        )
        unsuccess_response_check(response, status_code=404, description='Not Found (team)')

    def test_remove_user_from_nonexistent_team(self, api_client, unsuccess_response_check, random_str,
                                               service_user_creating):
        """Test removing user from non-existent team"""
        fake_team_id = f'nonexistent-{random_str()}'
        service_user = service_user_creating()

        response = remove_user_from_team.sync_detailed(
            client=api_client,
            id=fake_team_id,
...
class TestTeamPerformance:
    """Test suite for team operation performance scenarios"""

    def test_multiple_team_operations_performance(self, api_client, successful_response_check, random_str):
        """Test performance of multiple team operations"""
        team_ids = []

        # Create multiple teams
        for i in range(5):
            response = create_team.sync_detailed(
                client=api_client,
                body=TeamPayload(name=f'perf-team-{i}-{random_str()}')
            )
            successful_response_check(response, description='Team info (created)')
            team_ids.append(response.parsed.team.id)

            # List teams and verify all are present
        response = get_teams.sync_detailed(client=api_client)
        successful_response_check(response, description='Teams Information')
        greater(len(response.parsed.teams), 4)  # At least our 5 teams
