...
export class SocketWrapper extends EventEmitter {
     ...
     */
    send(args) {
        // Prevent infinite promise chain growth by periodically resetting
        // After every 1000 messages, we reset the chain to avoid memory accumulation
        this.#sendQueueLength++

        if (this.#sendQueueLength > 1000) {
            // Wait for current queue to drain, then reset
            this.#sendQueue = this.#sendQueue
                .catch(() => {}) // Ignore any previous errors
                .then(() => {
            ...
            this.#sendQueue = this.#sendQueue
                .catch(() => {}) // Don't let failed sends break the chain
                .then(() => this.sendAsync(args))
        }
