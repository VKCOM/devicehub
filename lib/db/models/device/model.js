...
export const loadDevicesPresentBefore = function(date) {
    return db.devices.find(
        {
            present: true,
            presenceChangedAt: {$lt: date}
        }
    ).project({serial: 1, 'provider.name': 1}).toArray()
}
...
export const generateIndexes = function() {
    ...
    // Backs the presence-by-age queries (getDeadDevice, loadDevicesPresentBefore),
    // which would otherwise be collection scans.
    db.devices.createIndex({present: 1, presenceChangedAt: 1}).then((result) => {
        log.info('Created presence indexes with result - ' + result)
    })
}
