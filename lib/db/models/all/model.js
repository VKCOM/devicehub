...
export const updateDevicesOriginGroup = async(serial, group) => {
    ...
    if (stats.modifiedCount) {
        log.info('[updateDevicesOriginGroup] Successfully updated origin group in device [serial: "%s", group: "%s", name: "%s"]', serial, group.id, group.name)
    }
    else {
        log.error('[updateDevicesOriginGroup] Device not found [serial: "%s", group: "%s", name: "%s"]', serial, group.id, group.name)
    }
