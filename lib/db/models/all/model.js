...
export const updateDevicesOriginGroup = async(serial, group) => {
    const updateDevice = async(deviceSerial) => {
        const device = await db.devices.findOne({serial: deviceSerial})
        if (!device) {
            return false
        }

        const previousOrigin = device.group?.origin
        const isInOriginGroup = device.group?.id === previousOrigin
        const set = {
            'group.origin': group.id,
            'group.originName': group.name
        }

        // Keep a free device visible in its newly assigned origin group. A device
        // in a transient booking must remain in that booking until it is released.
        if (isInOriginGroup) {
            Object.assign(set, {
                'group.id': group.id,
                'group.name': group.name,
        ...
        const stats = await db.devices.updateOne(
            {serial: deviceSerial, 'group.origin': previousOrigin, 'group.id': device.group?.id},
            {$set: set}
        )

        return stats.modifiedCount || stats.matchedCount
    }

    const results = await Promise.all((Array.isArray(serial) ? serial : [serial]).map(updateDevice))
    ...
    log.error(
        '[updateDevicesOriginGroup] Device(s) not found [serial: "%s", group: "%s", name: "%s"]',
        serial,
        group.id,
        group.name
    )
