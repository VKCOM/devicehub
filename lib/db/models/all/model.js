...
export const updateDevicesOriginGroup = async(serial, group) => {
    ...
    if (stats.modifiedCount) {
        log.info('[updateDevicesOriginGroup] Successfully updated origin group [serial: "%s", group: "%s", name: "%s"]', serial, group.id, group.name)
    } else {
        log.error('[updateDevicesOriginGroup] Group not found [serial: "%s", group: "%s", name: "%s"]', serial, group.id, group.name)
    }
