...
export const askUpdateDeviceOriginGroup = async(serial, group, signature) => {
    ...
    if (stats.modifiedCount) {
        log.info('[askUpdateDeviceOriginGroup] Successfully set group ticket [serial: "%s", group: "%s"]', serial, group.id)
        GroupChangeHandler.doUpdateDeviceOriginGroup(Object.assign({}, group, data))
    }
    else {
        log.error('[askUpdateDeviceOriginGroup] Group not found [serial: "%s", group: "%s", stats: %s]', serial, group.id, JSON.stringify(stats, null, 2))
    }
