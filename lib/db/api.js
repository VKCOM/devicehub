...
const findWithFields = function(collection, condition, fields) {
    return collection.find(condition, fields).toArray()
}

const findOneWithFields = function(collection, condition, fields) {
    return collection.findOne(condition, fields)
}

const findDevice = function(condition, fields) {
    return db.connect().then(async client => {
        const collection = client.collection('devices')
        if (Object.keys(condition).includes('serial')) {
            return await findOneWithFields(collection, condition, fields)
        }
        return await findWithFields(collection, condition, fields)
    })
}

// dbapi.loadDevices = function(groups) {
export const loadDevices = function(groups, fields) {
    return trace('loadDevices', {groups, fields}, () => {
