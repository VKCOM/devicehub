...
export const builder = function(yargs) {
    return yargs
        ...
        .option('adb-autostart', {
            describe: 'Automatically (re)start the ADB server if it is down or crashes. ' +
            'Only works when the ADB server runs on this host. Disabled by default.',
            type: 'boolean',
            default: false
        })
        .option('adb-path', {
            describe: 'Path to the adb binary used by --adb-autostart. ' +
            'Defaults to looking up `adb` in PATH.',
            type: 'string',
            default: 'adb'
        })
        .option('idle-device-reconnect', {
            describe: 'Force a reconnect of a device after it has been idle ' +
            '(not in use) for the specified number of minutes. 0 disables the feature.',
            type: 'number',
            default: 0
        })
        .option('ios-host', {
        ...
        .option('bind-proxy-router', {
            describe: 'The address to bind the proxy ZeroMQ ROUTER endpoint to ' +
            '(the single public entrypoint for all app-side units). NOTE: must not ' +
            'collide with the websocket HTTP port (7110) — 7150 is used here.',
            type: 'string',
            default: 'tcp://127.0.0.1:7150'
        })
        .option('bind-processor-router', {
            describe: 'The address to bind the processor device-facing ZeroMQ ' +
            'ROUTER endpoint to (providers/devices connect here).',
            type: 'string',
            default: 'tcp://127.0.0.1:7160'
        })
        .option('cleanup', {
        ...
        .option('without-provider', {
            describe: 'Start without provider unit',
            type: 'boolean',
            default: false
        })
        .option('without-poorxy', {
            describe: 'Start without poorxy unit',
            type: 'boolean',
            default: false
        })
...
export const handler = function(argv) {
    ...
    const stages = [
        ...
        [ // stage 1
            [ // proxy (single ROUTER: the public entrypoint for all app-side units)
                'proxy', 'proxy001',
                '--bind-router', argv.bindProxyRouter
            ]
        ],
        [ // stage 2
            [ // processor one: DEALER -> proxy, ROUTER <- providers/devices
                'processor', 'proc001',
                '--connect-proxy', argv.bindProxyRouter,
                '--bind-router', argv.bindProcessorRouter,
            ...
            [ // groups engine: single DEALER -> proxy
                'groups-engine',
                '--connect-proxy', argv.bindProxyRouter
            ],
            [ // websocket: single DEALER -> proxy (broadcast receiver)
                'websocket',
    ...
    if (!argv.withoutProvider) {
        stages[stages.length - 1].push(
            [ // provider
                ...
                '--connect-processor', argv.bindProcessorRouter,
                ...
                '--adb-path', argv.adbPath,
                '--idle-device-reconnect', argv.idleDeviceReconnect,
                ...
                .concat(argv.adbAutostart ? ['--adb-autostart'] : [])
