...
export const builder = function(yargs) {
    return yargs
        ...
        .option('connect-processor', {
            alias: 'p',
            describe: 'Processor ZeroMQ ROUTER endpoint the forked ios-devices connect to.',
            array: true,
            demand: true
        })
...
export const handler = function(argv) {
    ...
    argv.connectProcessor = argv.connectProcessor.filter(s => !!s.trim().length)
    ...
    return provider({
        ...
        fork: (serial, opts) => {
            const args = [
                ...
                ... argv.connectProcessor.reduce((all, val) => all.concat(['--connect-processor', val]), []),
