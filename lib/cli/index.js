...
yargs(hideBin(process.argv)).usage('Usage: $0 <command> [options]')
    ...
    .fail((msg, err) => {
        ...
        const foundKeys = msg.replace(/Unknown argument(s)?: /, '').split(', ')

        const realBadCliArgs = foundKeys.filter(key =>
        ...
        if (realBadCliArgs.length === 0) {
            return
        }

        const finalMsg = realBadCliArgs.length === 1 ?
            `Unknown argument: ${realBadCliArgs[0]}` :
            `Unknown arguments: ${realBadCliArgs.join(', ')}`
