...
class DeviceSubscriber {
    ...
    /** @private {Map<string, {responseChannel: string, timeout: NodeJS. Timeout}>} */
    static map = new Map()

    /**
     * @param {string} device
     * @param {SocketWrapper} sub
     * @return {string} responseChannel
     * */
    static subscribe(device, sub) {
        const existed = this.map.get(device)
        if (existed) {
            if (existed.timeout) {
                clearTimeout(existed.timeout)
            }
            existed.timeout = 0
            return existed.responseChannel
        }

        const responseChannel = 'txn_' + uuidv4()
        sub.subscribe(responseChannel)

        this.map.set(device, {responseChannel, timeout: 0})
    ...
    static unsubscribe(device, sub) {
        ...
        existed.timeout = setTimeout(() => {
            sub.unsubscribe(this.map.get(device).responseChannel)
 ...
 */
const useDevice = ({user, device, channelRouter, push, sub, usage = null, log}) => new Promise((resolve, reject) => {
    if (!device) {
    ...
    const messageListener = new WireRouter()
        .on(wire.ConnectStartedMessage, async(channel, message) => {
            if (!connectTimeoutId) {
                await new Promise(r => setTimeout(r, 650))
            }

            if (connectTimeoutId && message.serial === device.serial) {
    ...
    const useDeviceMessageListener = new WireRouter()
        .on(wire.JoinGroupMessage, function(channel, message) {
            ...
            if (message.serial === device.serial && message.owner.email === user.email) {
                ...
                connectTimeoutId = connectTimeout()
                push.send([
                    device.channel,
    ...
    push.send([
        device.channel,
        wireutil.envelope(new wire.UngroupMessage(deviceRequirements))
    ])
    push.send([
        device.channel,
