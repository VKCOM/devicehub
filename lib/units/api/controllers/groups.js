...
import {loadDevicesBySerials} from '../../../db/models/all/model.js'
    ...
    function _addGroupDevices(group, serials) {
        return dbapi.addGroupDevices(group, serials)
            .then(async(autotestsGroup) => {
                ...
                if (isInternal) {
                    return loadDevicesBySerials(autotestsGroup?.devices)
...
function getGroups(req, res) {
    ...
    try {
        ...
        apiutil.internalError(res, 'Failed to get groups: ', err?.stack || err)
    }
}
        ...
        return dbapi.updateUserGroup(group, data)
            .then(function(group) {
                if (group && typeof group !== 'boolean') {
                    if (isInternal) {
                        loadDevicesBySerials(group.devices)
        ...
        return checkSchedule(res, group, _class, group.owner.email, repetitions, group.privilege, start, stop)
            .then(function(checkingSuccessed) {
                ...
                if (
                    ...
                    if (isInternal) {
                        return loadDevicesBySerials(group.devices)
