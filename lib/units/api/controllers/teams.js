...
async function groupBulkOperations(req, res, operation) {
    ...
    if (operation === 'delete') {
        for (const user of users) {
            await dbapi.removeUserFromTeam(teamId, user)
        }
        for (const group of groups) {
...
export {getTeamById, createTeam, updateTeam, removeFromTeam, removeUserFromTeam, removeGroupFromTeam, getUserTeams, getGroupsTeams, deleteTeam}
export default {
