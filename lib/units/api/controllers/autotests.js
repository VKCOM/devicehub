...
function installOnDevice(req, res) {
    ...
    return dbapi.loadDeviceBySerial(serial).then(async device => {
        if (device === null) {
            res.status(404).json({
                success: false,
        ...
        log.info('Sending InstallMessage to %s/%s', device.provider.name, device.serial)
        try {
            ...
            log.info('Installation result: ' + result.data)
            return res.json({
                success: true,
            ...
            log.info('Installation failed: ' + (err?.data ?? err?.message ?? err))
            return res.status(400).json({
                success: false,
                description: err?.data ?? 'Installation failed'
            })
        }
    })
}
...
async function useAndConnectDevice(req, res) {
    ...
    try {
        ...
        const remoteConnectUrl = await useDevice({
            ...
            usage: 'automation',
            log,
            runTransaction: (mt, m, opts) =>
                req.options.txmanager.runTransaction(device.provider.name, device.serial, mt, m, opts),
        })
