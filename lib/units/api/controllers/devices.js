...
// Note: Device lists in groups change automatically
const addDeviceToOriginGroup = async(req, res) => {
    try {
        const serial = req.query.serial || req.params.serial
        const fields = req.query.fields

        if (!serial?.length) {
            throw new Error('serial is not specified')
        }

        const targetGroup = await dbapi.getUserGroup(req.user.email, req.params.id)
        if (!targetGroup) {
            throw new Error('target group not found')
        }

        const stats = await dbapi.lockDeviceByOrigin(req.user.groups.subscribed, serial)
        if (stats.modifiedCount === 0) {
            return apiutil.lightComputeStats(res, stats)
        }

        const signature = uuidv4().replace(/-/g, '')
        await dbapi.updateDeviceOriginGroup(serial, targetGroup, signature)

        lockutil.unlockDevice({device: stats.changes[0].new_val})

        const device = await dbapi.loadDeviceBySerial(serial)
        return apiutil.respond(res, 200, 'Updated (device)', apiutil.publishDevice(device, req))
    }
    catch (/** @type {any} */e) {
        log.error('[addDeviceToOriginGroup] failed to update device origin group: %s', e?.message)
        apiutil.internalError('Failed to update device origin group')
    }
}

// Note: Device lists in groups change automatically
const removeDeviceFromOriginGroup = async(req, res) => {
    try {
        ...
        const targetGroup = await dbapi.getRootGroup()
        if (!targetGroup) {
            throw new Error('target group not found')
        }
...
export {addDeviceToOriginGroup}
export {removeDeviceFromOriginGroup}
export {deleteDevice}
...
export default {
    ...
    addDeviceToOriginGroup: addDeviceToOriginGroup,
    removeDeviceFromOriginGroup: removeDeviceFromOriginGroup,
