...
import {DeleteUserMessage} from '../../../wire/wire.js'
    ...
    function deleteUserInDatabase(channel) {
        return AllModel.removeUserAccessTokens(email).then(function() {
            return UserModel.deleteUser(email).then(function() {
                // Broadcast the deletion; the websocket unit disconnects the
                // affected user's sockets (it filters by email client-side).
                req.options.transport.sendBroadcast(wireutil.pack(DeleteUserMessage, {email}))
                return 'deleted'
            })
        })
    }
