...
import {KeyDownMessage, KeyPressMessage, KeyUpMessage, PhysicalIdentifyMessage, RotateMessage, TypeMessage, UnlockDeviceMessage} from '../../../wire/wire.js'
...
export default syrup.serial()
    ...
    .define(async function(options, adb, router, push, sdk, apk, devutil) {
        ...
        await openAgent()
            ...
            .then(function() {
                router
                    .on(PhysicalIdentifyMessage, function(channel) {
                    ...
                    .on(KeyPressMessage, function(channel, message) {
                        try {
                            keyEvent({
                                event: apk.wire.KeyEvent.PRESS,
                                keyCode: keyutil.namedKey(message.key)
                    ...
                    .on(RotateMessage, function(channel, message) {
                        plugin.rotate(message.rotation)
                    })
                    .on(UnlockDeviceMessage, function(channel, message) {
                        plugin.unlockDevice()
                    })
            })
        return plugin
