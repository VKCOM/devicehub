    ...
    .define(function(options, adb, router, minitouch, flags) {
        ...
        TouchConsumer.prototype.touchDown = function(point) {
            this._queueWrite(function() {
                return this._write(util.format('d %s %s %s %s\n', point.contact, Math.ceil(this.touchConfig.origin.x(point) * this.banner.maxX), Math.ceil(this.touchConfig.origin.y(point) * this.banner.maxY), Math.ceil((point.pressure || 0.5) * this.banner.maxPressure)))
            })
        }
        TouchConsumer.prototype.touchMove = function(point) {
            this._queueWrite(function() {
                return this._write(util.format('m %s %s %s %s\n', point.contact, Math.ceil(this.touchConfig.origin.x(point) * this.banner.maxX), Math.ceil(this.touchConfig.origin.y(point) * this.banner.maxY), Math.ceil((point.pressure || 0.5) * this.banner.maxPressure)))
            })
        }
        ...
        TouchConsumer.prototype._ensureState = function() {
            ...
            case TouchConsumer.STATE_STOPPED:
                if (this.desiredState.next() === TouchConsumer.STATE_STARTED) {
                    ...
                    this.starter = this._startService()
                        .then((out) => {
        ...
        TouchConsumer.prototype._startService = async function() {
            return await minitouch.run()
        }
        ...
        TouchConsumer.prototype._connectService = function() {
            function tryConnect(times, delay) {
                return adb.getDevice(options.serial).openLocal('localabstract:minitouch')
                    .then(function(out) {
                        return out
                    })
                    .catch(function(err) {
        ...
        TouchConsumer.prototype._readBanner = async function(socket) {
