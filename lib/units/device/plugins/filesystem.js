...
export default syrup.serial()
    ...
    .dependency(storage)
    .define(function(options, adb, router, transport, storage) {
        ...
        router.on(FileSystemGetMessage, function(channel, message) {
            ...
            plugin.retrieve(message.file, message.jwt)
                ...
                .catch(function(err) {
                    log.warn('Unable to retrieve "%s": %s', message.file, err.message)
                    transport.send([
                        channel,
        ...
        router.on(FileSystemListMessage, function(channel, message) {
            ...
            adb.getDevice(options.serial).readdir(message.dir)
                ...
                .catch(function(err) {
                    log.warn('Unable to list directory "%s": %s', message.dir, err.message)
                    transport.send([
                        channel,
                        reply.fail(err.message)
                    ])
