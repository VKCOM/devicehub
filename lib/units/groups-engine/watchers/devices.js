 ...
 */
export default (function(transport, txmanager) {
    ...
    async function sendReleaseDeviceControlAndDeviceGroupChange(device, sendDeviceGroupChangeWrapper) {
        // Address the device by (provider.name, serial); the Ungroup transaction
        // resolving is the release confirmation.
        if (device?.provider?.name) {
            try {
                await txmanager.runTransaction(device.provider.name, device.serial, UngroupMessage, {
            ...
            log.warn('Cannot release control of %s: no provider', device?.serial)
        }
